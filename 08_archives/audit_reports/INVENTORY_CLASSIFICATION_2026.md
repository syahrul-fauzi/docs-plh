...
Audit ini dilakukan untuk memetakan seluruh artefak yang ada di repository `lawyers-hub` guna mendukung transisi ke arsitektur dokumentasi enterprise.
...
- **Gap**: Dokumentasi alur bisnis (Business Process) belum terformalisasi secara visual (hanya teks).
- **Redundansi**: Beberapa file `TECHNICAL_SPEC.md` dan `TECHNICAL_SPECIFICATION.md` di root `docs/` tumpang tindih.
- **Inkonsistensi**: Penamaan file antara kebab-case dan PascalCase masih bercampur.
...
3. Standarisasi penamaan file menjadi `snake_case` atau `kebab-case` sesuai Style Guide.
