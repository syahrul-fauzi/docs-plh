...
Dokumen ini mendefinisikan arsitektur sistem berbasis **Turborepo + Next.js +
NestJS** yang dirancang untuk skala produksi, mendukung multi-tenancy,
mengintegrasikan komponen AI canggih, dan mengikuti prinsip Clean Architecture.
...
(Single Responsibility) untuk memastikan _testability_.
...
- **Database (Logical Isolation)**: Menggunakan `tenant_id` pada setiap tabel
  dengan _Row Level Security (RLS)_ di PostgreSQL.
- **Dynamic Connection Pooling**: NestJS middleware yang menyuntikkan
  `tenant_id` ke dalam Prisma context.
...
AI divalidasi oleh `packages/rules-engine` yang berisi logika hukum
deterministik untuk mengurangi halusinasi.
