...
2026-01-03 **Owner:** Engineering Team

---

## 1. Overview

The Notification System is a critical component for user engagement and
compliance enforcement. It ensures that users are timely informed about:
...
- **EmailService**: Centralized service for sending emails via SMTP
  (Nodemailer).
- **BillingSchedulerService**: Cron-based service (using `@nestjs/schedule`) to
  run periodic checks.
- **Event Triggers**: Direct calls from Feature Services (e.g., `DraftsService`,
  `InvitationsService`).
...
- **Content**: Warning about downgrade to FREE plan and link to billing
  dashboard.
...
- **Persistence**: Database storage (`Notification` model) for "Bell" icon
  history.
...
_This document governs the implementation of the Notification System._
