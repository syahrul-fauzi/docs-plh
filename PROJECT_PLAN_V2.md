...
### Requirements:
- **Multi-tenancy**: Data isolation at DB level (RLS) and Vector level (Namespaces).
...
- **Scalability**: Microservices-ready backend with BullMQ for heavy tasks (OCR, RAG).
...
2.  **Prerequisite**: PostgreSQL must be accessible with `DATABASE_URL` configured.
3.  **Verification**:
    *   Execute `SELECT * FROM pg_policies;` to confirm policies for all tables.
    *   Test isolation: `SET app.current_tenant = 'uuid'; SELECT * FROM documents;` should only return tenant data.

### RAG & Embedding Integration
Transitioning from mock to live embeddings:
1.  **Service**: `EmbeddingService` in `@lawyers-hub/ai`.
2.  **Provider**: OpenAI `text-embedding-3-small` (Recommended for cost/performance).
3.  **Security**: API keys managed via environment variables; PII masking enforced before embedding.

### Monitoring & Quality Control
1.  **Metrics**: Prometheus instrumentation in `LegalRulesEngine` (latency, compliance rate).
2.  **E2E Testing**: Playwright tests for real-time collaboration and RAG accuracy.
