...
Mesin validasi deterministik untuk memastikan output AI sesuai dengan batasan
hukum.
...
- **PII Masking**: Semua query AI (`generateResponse`) wajib melewati `maskPII`
  sebelum dikirim ke LLM.
- **Audit Trail**: Setiap tindakan AI dicatat dalam `AuditLog` dan `PIIAuditLog`
  (database).

### 2.2 Database & Multitenancy

- **Tenant Isolation**: Semua query database wajib menyertakan `tenantId`.
- **Compliance Sync**: Hasil `ComplianceReview` disinkronkan secara periodik
  untuk retraining model NER.
