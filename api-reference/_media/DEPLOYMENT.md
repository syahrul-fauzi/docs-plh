...
## Strategi Deployment
Lawyers Hub menggunakan strategi **Blue-Green Deployment** untuk meminimalkan downtime selama rilis fitur baru.
...
2.  **Database Migration**: Jalankan `npx prisma db push` (atau `migrate deploy` di produksi).
...
- Jika error rate > 5% selama 10 menit, sistem akan otomatis melakukan rollback ke image sebelumnya.
